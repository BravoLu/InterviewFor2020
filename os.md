...
* 信号量，共享内存，消息队列关注进程间通信
* 条件变量，互斥锁，主要关注线程间通信，条件变量特别适用于多个线程等待某个条件的发生。如果不使用条件变量，那么每个线程就需要不断尝试获得互斥锁并检查条件是否发生，这样大大浪费了系统的资源。
...
* 僵尸进程：子进程运行结束后，父进程没有调用wait/waitpid（waitpid是wait的封装，多两个可由用户控制的参数pid和options）函数，导致出现僵尸进程。当父进程结束后，僵尸进程变为孤儿进程，孤儿进程被init进程回收。

## 守护进程简单理解

* 将后台程序变成一种服务，比如说，用命令行输入启动程序，如果不是守护进程的话，一旦命令行窗口关闭，程序就终止了；而如果启动守护进程，则退出命令行窗口之后，服务一直处于运行状态。 
* 与后台进程的区别：
  * 守护进程已经完全脱离终端控制台了，而后台程序并没有完全脱离终端
  * 守护进程在关闭终端控制台时不受影响，而后台程序会随用户退出而停止，需要以nohup comman & 格式运行才能避免影响。
...
* 消息队列和命名管道很相似
  ...
  * 可以同时通过发消息以避免命名管道的同步和阻塞问题，而不需要由进程自己来提供同步方法。
  * 接收程序可以通过消息类型有选择地接收数据，而不是像命名管道那样默认接收。
